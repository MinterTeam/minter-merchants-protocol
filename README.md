...
### Merchant
После попадания пользователя в магазин на страницу оплаты, мерчант получает данные о заказе [из query параметров](#payment-url-query-parameters): id оффера, чек и пароль для обналичивания, публичный ключ для шифрования пейлоада.
...
```json
{
  ...
  "provider": {
    ...
    "payment_url": "https://minterpush.com/mmp-payment",
...
- `payment_url`: урл куда будет перенаправляться юзер с данными для оплаты в query параметрах
...
## Список фидов наполняемый мерчантами
...
## Payment URL query parameters 
Параметры передаваемые на страницу оплаты мерчанту
...
```
https://minterpush.com/mmp-payment?offer_id=123&check=&password=fcViycYgwea4m-iz&public_key=0x66c808e6b5be6d6620934bc6ffa2b8b47f9786c002bfb06d53a0c27535641a5d1
```



## Callback to user
Мерчант посылает уведомление в случае завершения обработки заказа.  
...
пример:
```
mmp1.0.0 0x123... ASdasdASDASdasd...
```

- `mmp{{version}}` - заголовок с версией протокола
...
- мерчант получает от юзера compressed публичный ключ длиной 33 байта, восстанавливает из него uncompressed (65 байт)
- для каждого сообщения генерирует свою новую пару приватный-публичный ключ
...
- из общего секрета получает секретный ключ для шифрования с помощью [HKDF](https://en.wikipedia.org/wiki/HKDF). В качестве функция хэширования используется SHA512.
- зашифровывает сообщение секретным ключом с использованием симметричного шифрования AES256 CBC
- отправляет пользователю свой сжатый публичный ключ и зашифрованное сообщение
...
//@TODO написать JSON-Schema для всех данных https://extendsclass.com/json-schema-validator.html

//@TODO список доступных категорий

...
- [Payment URL query parameters](#payment-url-query-parameters)
...
#### Field role Quantity
`provider` -> `offer_list` -> `field_list` -> `role: quantity`
...
#### Field role Phone
`provider` -> `offer_list` -> `field_list` -> `role: phone`

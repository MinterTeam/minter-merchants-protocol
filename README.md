...
- [Payment URL query parameters](#payment-url)
...
### Merchant
После попадания пользователя в магазин на страницу оплаты, мерчант получает данные о заказе [из query параметров](#payment-url): id оффера, чек и пароль для обналичивания, публичный ключ для шифрования пейлоада.
...
- `payment_url`: [страница оплаты](#payment-url), куда будет перенаправляться юзер для, данные заказа будут переданы в query параметрах, 
...
## Payment URL
Query параметры передаваемые на страницу оплаты мерчанту
